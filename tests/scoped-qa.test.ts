...
import { runScopedQa } from "../src/qa/run-scoped.js";
...
describe("runScopedQa", () => {
  it("drives only the unverified capabilities a change touched", async () => {
    const driven: string[] = [];
    const deps: QaDeps = { drive: fakeDrive(driven), compiler: new FakeCompiler(), runner: new FakeRunner("passed") };
  ...
  it("records an error when a capability has no start URL", async () => {
    const deps: QaDeps = { drive: fakeDrive([]), compiler: new FakeCompiler(), runner: new FakeRunner("passed") };
    const result = await runScopedQa(deps, {
      graph: GRAPH,
      config: CONFIG,
